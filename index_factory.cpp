...
Index *index_factory (int d, const char *description_in, MetricType metric)
{
    ...
    std::vector<char> description(strlen(description_in) + 1);
    char *ptr;
    memcpy (description.data(), description_in, strlen(description_in) + 1);

    int64_t ncentroids = -1;
    bool use_2layer = false;
    int hnsw_M = -1;

    for (char *tok = strtok_r (description.data(), " ,", &ptr);
         tok;
